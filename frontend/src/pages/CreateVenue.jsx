...
const CreateVenue = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (from) formData.append("availableFrom", from.toISOString());
            if (to) formData.append("availableTo", to.toISOString());
            images.forEach((img) => formData.append("images", img));

            await axios.post("http://localhost:3000/api/owners/create", formData, {
                headers: {
            ...
            console.error(err);
            toast.error(err.response?.data?.msg || "Failed to create venue");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-black via-neutral-900 to-black text-white relative">
            ...
            <form
                ...
                className="h-[95%] relative z-10 bg-black/50 backdrop-blur-xl border border-white/10 shadow-2xl rounded-2xl p-10 w-full max-w-4xl flex gap-8"
            >
                {/* Left Column: Inputs */}
                <div className="flex-1 flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                        <label className="text-sm text-gray-400">Venue Images (max 5)</label>
                        <input
                            type="file"
                            multiple
                            accept="image/*"
                            onChange={handleImageChange}
                            className="text-white bg-neutral-950 border border-gray-700 p-3 rounded-lg cursor-pointer"
                        />
                    </div>
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className={`cursor-pointer w-full py-1 rounded-lg font-semibold text-lg transition-all duration-200 ${loading
                ...
                <div className="flex-1 flex flex-col gap-4 overflow-y-auto max-h-[600px]">
                    {previews.length > 0 ? (
                        previews.map((src, index) => (
                            <div key={index} className="w-full h-40 rounded-lg overflow-hidden border border-gray-600">
